...
@section('title', 'Giỏ hàng - FlashTech Premium')
                    ...
                    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                        ...
                        <div class="flex items-center justify-between px-6 sm:px-8 py-6 border-b border-slate-100 bg-slate-50/50">
                            <h3 class="font-bold text-slate-800 flex items-center gap-2">
                                Sản phẩm 
                                <span class="bg-indigo-100 text-indigo-700 text-xs px-2.5 py-1 rounded-lg" id="itemCount">{{ count($cartItems) }}</span>
                            </h3>
                        ...
                        <div class="flex flex-col">
                            @foreach ($cartItems as $item)
                                <div class="group flex flex-col sm:flex-row gap-6 p-6 sm:p-8 border-b border-slate-50 last:border-0 hover:bg-indigo-50/30 transition-colors" id="cart-item-{{ $item['id'] }}">
                                    <!-- Image Image -->
                                    <a href="{{ route('product.detail', $item['product']->slug) }}" class="w-24 h-24 sm:w-32 sm:h-32 shrink-0 bg-slate-50 rounded-2xl border border-slate-100 p-2 overflow-hidden relative">
                                        @php 
                                            $img = $item['product']->image ?? ''; 
                                            $cartImg = empty($img) ? 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&h=200&fit=crop' : (Str::startsWith($img, 'http') ? $img : asset(Str::startsWith($img, '/storage/') ? $img : 'storage/' . $img));
                                        @endphp
                                        <img src="{{ $cartImg }}" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&h=200&fit=crop';" alt="{{ $item['product']->name }}" class="w-full h-full object-contain group-hover:scale-110 transition-transform duration-500">
                                    </a>
                                    ...
                                    <div class="flex-1 flex flex-col justify-between gap-4">
                                        <div class="flex justify-between items-start gap-4">
                                            <div>
                                                <a href="{{ route('product.detail', $item['product']->slug) }}" class="text-lg font-bold text-slate-800 hover:text-indigo-600 transition-colors line-clamp-2 leading-snug mb-2">{{ $item['product']->name }}</a>
                                                @if(!empty($item['color']))
                                                    <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-100 text-slate-600 text-xs font-bold border border-slate-200">
                                                        <span class="w-2 h-2 rounded-full shadow-inner border border-slate-300" style="background-color: {{ ['Đen'=>'#1a1a1a','Trắng'=>'#fff','Xanh'=>'#3b82f6','Đỏ'=>'#ef4444'][$item['color']] ?? '#9ca3af' }}"></span>
                                                        {{ $item['color'] }}
                                                    </span>
                                                @endif
                                            </div>
                                            <button onclick="removeItem('{{ $item['id'] }}')" class="shrink-0 p-2 text-slate-300 hover:text-rose-500 hover:bg-rose-50 rounded-xl transition-all" title="Xóa">
                                        ...
                                        <div class="flex flex-wrap items-end justify-between gap-4 mt-auto">
                                            ...
                                            <div class="flex flex-col gap-1">
                                                <span class="text-xl font-black text-indigo-600">{{ number_format($item['price'], 0, ',', '.') }}₫</span>
                                                @if ($item['product']->sale_price > 0 && $item['product']->sale_price < $item['product']->price)
                                                    <span class="text-xs font-bold text-slate-400 line-through">{{ number_format($item['product']->price, 0, ',', '.') }}₫</span>
                                                @endif
                                            </div>
                                            ...
                                            <div class="flex items-center gap-6">
                                                <div class="flex items-center bg-slate-50 border border-slate-200 rounded-xl overflow-hidden p-1">
                                                    <button class="w-8 h-8 flex items-center justify-center bg-white rounded-lg text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 shadow-sm transition-colors font-bold disabled:opacity-50" onclick="updateQuantity('{{ $item['id'] }}', -1)" id="dec-{{ $item['id'] }}">‒</button>
                                                    ...
                                                    @endphp
                                                    <input type="number" class="w-12 h-8 text-center bg-transparent font-bold text-slate-800 focus:outline-none" value="{{ $item['quantity'] }}" min="1" max="{{ $colorStock }}" id="qty-{{ $item['id'] }}" onchange="setQuantity('{{ $item['id'] }}')">
                                                    
                                                    <button class="w-8 h-8 flex items-center justify-center bg-white rounded-lg text-slate-600 hover:text-indigo-600 hover:bg-indigo-50 shadow-sm transition-colors font-bold disabled:opacity-50" onclick="updateQuantity('{{ $item['id'] }}', 1)" id="inc-{{ $item['id'] }}">+</button>
                                                </div>
                                                <div class="text-right min-w-[100px]">
                                                    <span class="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Thành tiền</span>
                                                    <span class="text-lg font-black text-slate-800" id="total-{{ $item['id'] }}">{{ number_format($item['total'], 0, ',', '.') }}₫</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-6 sm:p-8">
                        ...
                        <div class="flex flex-col gap-4 mb-6 text-sm font-medium">
                            <div class="flex justify-between items-center text-slate-600">
                                <span>Tạm tính</span>
                                <span class="font-bold text-slate-800" id="subtotalValue">{{ number_format($subtotal, 0, ',', '.') }}₫</span>
                            </div>
                            <div class="flex justify-between items-center text-slate-600">
                        ...
                        @if(auth('customer')->check())
                            <a href="{{ route('checkout.index') }}" class="group w-full flex items-center justify-center gap-2 bg-slate-900 text-white font-bold px-6 py-4 rounded-2xl border-2 border-slate-900 shadow-xl shadow-slate-900/20 hover:bg-indigo-600 hover:border-indigo-600 transition-all duration-300">
                        ...
                        @else
                            <a href="{{ route('customers.login') }}" class="w-full flex items-center justify-center gap-2 bg-slate-900 text-white font-bold px-6 py-4 rounded-2xl hover:bg-indigo-600 transition-all shadow-xl shadow-slate-900/20">
                                Đăng nhập để thanh toán
                            </a>
                        @endif

                        <a href="{{ route('home') }}" class="w-full flex items-center justify-center text-slate-500 font-bold px-6 py-4 rounded-2xl hover:bg-slate-50 hover:text-indigo-600 transition-colors mt-3 gap-2">
