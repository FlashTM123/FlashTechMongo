...
@section('title', 'Thanh toán - FlashTech Premium')
...
<div class="bg-slate-50 min-h-screen pb-20 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 pt-4 sm:pt-8 md:pt-12 relative z-10">
        ...
        <nav class="flex items-center gap-2 text-sm text-slate-400 mb-8 border border-white/20 bg-white/10 backdrop-blur-md w-fit px-4 py-2 rounded-full shadow-sm">
            ...
            <a href="{{ route('cart.index') }}" class="hover:text-indigo-600 transition-colors">Giỏ hàng</a>
            <span class="text-slate-300">/</span>
                ...
                <div class="lg:col-span-7 xl:col-span-8 flex flex-col gap-6">
                    ...
                    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-6 sm:p-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4">
                            ...
                            <label class="group relative cursor-pointer">
                                ...
                                <div class="flex items-center gap-4 p-4 border-2 border-slate-200 rounded-2xl bg-white transition-all peer-checked:border-indigo-600 peer-checked:bg-indigo-50/50 hover:border-indigo-300">
                                    ...
                                    <div class="w-5 h-5 rounded-full border-2 border-slate-300 flex items-center justify-center peer-checked:border-indigo-600 peer-checked:bg-indigo-600 transition-colors">
                                        <div class="w-2.5 h-2.5 rounded-full bg-white scale-0 transition-transform peer-checked:scale-100" style="display: {{ old('payment_method', 'cod') === 'cod' ? 'block' : 'none' }}"></div>
                                    </div>
                                </div>
                            </label>
                            ...
                            <!-- MoMo -->
                            <label class="group relative cursor-pointer">
                                <input type="radio" name="payment_method" value="momo" class="peer sr-only" {{ old('payment_method') === 'momo' ? 'checked' : '' }}>
                                <div class="flex items-center gap-4 p-4 border-2 border-slate-200 rounded-2xl bg-white transition-all peer-checked:border-indigo-600 peer-checked:bg-indigo-50/50 hover:border-indigo-300">
                                    <div class="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-2xl shadow-sm border border-slate-100">📱</div>
                                    <div class="flex-1">
                            ...
                            <!-- VNPay -->
                            <label class="group relative cursor-pointer">
                                <input type="radio" name="payment_method" value="vnpay" class="peer sr-only" {{ old('payment_method') === 'vnpay' ? 'checked' : '' }}>
                                <div class="flex items-center gap-4 p-4 border-2 border-slate-200 rounded-2xl bg-white transition-all peer-checked:border-indigo-600 peer-checked:bg-indigo-50/50 hover:border-indigo-300">
                                    <div class="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-2xl shadow-sm border border-slate-100">💳</div>
                                    <div class="flex-1">
                        ...
                        @error('payment_method')<span class="text-xs text-rose-500 mt-2 block">{{ $message }}</span>@enderror
                    </div>
                </div>
                ...
                <div class="lg:col-span-5 xl:col-span-4">
                    <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-6 sm:p-8 lg:sticky lg:top-24 h-fit">
                        <h3 class="text-xl font-black text-slate-800 mb-6 drop-shadow-sm border-b border-slate-100 pb-4">
                            Đơn Hàng <span class="text-slate-400 text-base font-medium">({{ count($cartItems) }} sản phẩm)</span>
                        </h3>
                            ...
                            @foreach ($cartItems as $item)
                                <div class="flex gap-4">
                                    <div class="w-16 h-16 shrink-0 bg-slate-50 rounded-xl border border-slate-100 p-1 relative">
                                        ...
                                        @endphp
                                        <img src="{{ $imgUrl }}" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&h=200&fit=crop';" alt="{{ $item['product']->name }}" class="w-full h-full object-contain">
                                        <span class="absolute -top-2 -right-2 w-5 h-5 bg-slate-800 text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-sm border border-white">{{ $item['quantity'] }}</span>
                                    </div>
                                    <div class="flex-1 flex flex-col justify-center">
                                        <h4 class="text-xs font-bold text-slate-800 line-clamp-2 leading-tight mb-1">{{ $item['product']->name }}</h4>
                                        @if(!empty($item['color']))
                                            <span class="text-[10px] text-slate-500 mb-1">Màu: {{ $item['color'] }}</span>
                                        @endif
                                        <span class="text-sm font-black text-indigo-600">{{ number_format($item['total'], 0, ',', '.') }}₫</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                            ...
                            @if ($appliedCoupon)
                                <div class="flex items-center justify-between bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-sm font-black text-emerald-800">{{ $appliedCoupon['code'] }}</span>
                                    </div>
                        ...
                        <div class="flex flex-col gap-3 mb-6 text-sm font-medium">
                            <div class="flex justify-between items-center text-slate-600">
                                <span>Tạm tính</span>
                                <span class="font-bold text-slate-800" id="subtotalDisplay">{{ number_format($subtotal, 0, ',', '.') }}₫</span>
                            </div>
                            
                            @if ($appliedCoupon)
                                <div class="flex justify-between items-center text-emerald-600 pb-3 border-b border-slate-100 border-dashed">
                                    <span>Giảm giá</span>
                                    <span class="font-bold" id="discountDisplay">-{{ number_format($appliedCoupon['discount'], 0, ',', '.') }}₫</span>
                                </div>
                            @else
                        ...
                        <div class="border-t border-slate-200 pt-6 mb-8">
                            <div class="flex justify-between items-end mb-2">
                                ...
                                <span class="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-purple-600" id="totalDisplay">
                                    {{ number_format($subtotal - ($appliedCoupon['discount'] ?? 0), 0, ',', '.') }}₫
                                </span>
                            </div>
                        ...
                        <a href="{{ route('cart.index') }}" class="w-full flex items-center justify-center text-slate-500 font-bold px-6 py-4 rounded-2xl hover:bg-slate-50 hover:text-indigo-600 transition-colors mt-3 gap-2">
