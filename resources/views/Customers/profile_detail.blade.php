...
@section('title', 'Thông tin cá nhân - FlashTech Premium')
        ...
        <div class="bg-white/70 backdrop-blur-xl rounded-[2.5rem] shadow-2xl shadow-indigo-500/10 border border-white p-6 sm:p-10 mb-8 relative">
            ...
            <div class="flex flex-col md:flex-row gap-8 items-center md:items-start relative z-10">
                ...
                <div class="relative shrink-0">
                    <div class="w-32 h-32 sm:w-40 sm:h-40 rounded-full border-8 border-white shadow-xl shadow-slate-200/50 bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center overflow-hidden">
                        @if($customer->profile_picture_url)
                            <img src="{{ $customer->profile_picture_url }}" alt="{{ $customer->full_name }}" class="w-full h-full object-cover">
                        @else
                            <span class="text-5xl font-black text-white">{{ substr($customer->full_name, 0, 1) }}</span>
                        @endif
                    </div>
                ...
                <div class="flex-1 text-center md:text-left mt-2">
                    <h1 class="text-3xl sm:text-4xl font-black text-slate-900 mb-2">{{ $customer->full_name }}</h1>
                    ...
                    <div class="flex flex-wrap justify-center md:justify-start gap-3">
                        <a href="{{ route('customers.profile.edit') }}" class="group flex items-center gap-2 bg-indigo-600 text-white font-bold px-6 py-3 rounded-xl shadow-lg shadow-indigo-500/30 hover:bg-slate-900 transition-all">
                        ...
                        <a href="{{ route('customers.password.change') }}" class="flex items-center gap-2 bg-white text-slate-700 font-bold px-6 py-3 rounded-xl border border-slate-200 shadow-sm hover:border-indigo-600 hover:text-indigo-600 transition-all">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-10 p-4 bg-slate-50/50 rounded-3xl border border-slate-100">
                <div class="bg-white rounded-2xl p-4 flex items-center justify-center gap-4 border border-slate-100/50 shadow-sm hover:-translate-y-1 transition-transform">
                    ...
                    <div class="text-left">
                        <div class="text-3xl font-black text-slate-800">{{ $ordersCount ?? 0 }}</div>
                ...
                <div class="bg-white rounded-2xl p-4 flex items-center justify-center gap-4 border border-slate-100/50 shadow-sm hover:-translate-y-1 transition-transform">
                    ...
                    <div class="text-left">
                        <div class="text-3xl font-black text-slate-800">{{ number_format($customer->loyalty_points ?? 0) }}</div>
                ...
                <div class="bg-white rounded-2xl p-4 flex items-center justify-center gap-4 border border-slate-100/50 shadow-sm hover:-translate-y-1 transition-transform">
                    ...
                    <div class="text-left">
                        <div class="text-3xl font-black text-slate-800">{{ $reviewsCount ?? 0 }}</div>
            ...
            <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 h-full flex flex-col group">
                ...
                <div class="flex flex-col gap-6 flex-1">
                    <div class="flex justify-between items-center sm:items-start flex-col sm:flex-row gap-1">
                        ...
                        <span class="font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-lg tracking-widest">{{ $customer->customer_code }}</span>
                    </div>
                    ...
                    <div class="flex justify-between items-start flex-col sm:flex-row gap-1">
                        ...
                        <span class="font-bold text-slate-800 sm:text-right">{{ $customer->address ?? '— Chưa cập nhật' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-8 h-full flex flex-col group">
                ...
                <div class="flex flex-col gap-6 flex-1">
                    ...
                    <div class="flex justify-between items-center sm:items-start flex-col sm:flex-row gap-1 pb-4 border-b border-slate-50">
                        ...
                        <span class="font-bold text-slate-800">{{ $customer->created_at->format('d/m/Y') }}</span>
                    </div>
                    <div class="flex justify-between items-center sm:items-start flex-col sm:flex-row gap-1">
                        <span class="text-sm font-medium text-slate-500 min-w-[120px]">Cập nhật cuối</span>
                        <span class="font-bold text-slate-800">{{ $customer->updated_at->format('d/m/Y H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
