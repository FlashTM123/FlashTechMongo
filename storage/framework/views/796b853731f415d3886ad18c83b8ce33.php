<?php $__env->startSection('title', $categoryInfo['name'] . ' - FlashTech Premium'); ?>
...
<div class="bg-slate-50 min-h-screen pb-20 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 pt-4 sm:pt-8 md:pt-12 relative z-10 max-w-7xl">
        ...
        <nav class="flex items-center gap-2 text-sm text-indigo-100 mb-8 bg-white/10 backdrop-blur-md w-fit px-4 py-2 rounded-full border border-white/20 shadow-sm">
            <a href="<?php echo e(route('home')); ?>" class="hover:text-white transition-colors flex items-center gap-1.5">
            ...
            <span class="text-white font-bold"><?php echo e($categoryInfo['name']); ?></span>
        </nav>
        ...
        <div class="flex items-center justify-between mb-10 border-l-4 border-indigo-400 pl-4 py-2">
            <div>
                <h1 class="text-3xl sm:text-4xl font-black text-white leading-tight mb-2 drop-shadow-md flex items-center gap-3">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php switch($categoryInfo['slug']):
                        case ('smartphone'): ?> 📱 <?php break; ?>
                        <?php case ('laptop'): ?> 💻 <?php break; ?>
                        <?php case ('tablet'): ?> 📲 <?php break; ?>
                        <?php case ('computer'): ?> 🖥️ <?php break; ?>
                        <?php case ('accessory'): ?> 🎧 <?php break; ?>
                        <?php default: ?> 📦
                ...
                <p class="text-indigo-100 font-medium tracking-wide">Khám phá <?php echo e($categoryInfo['count']); ?> sản phẩm công nghệ đỉnh cao</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <aside class="lg:col-span-1">
                <form action="<?php echo e(route('products.category', $categoryInfo['slug'])); ?>" method="GET" id="filterForm" class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-6 sm:p-8 sticky top-24">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="relative">
                            <input type="text" name="search" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all text-sm font-medium text-slate-700 placeholder:text-slate-400" placeholder="Nhập tên sản phẩm..." value="<?php echo e(request('search')); ?>">
                        </div>
                    </div>
                    ...
                    <div class="mb-8 border-t border-slate-100 pt-6">
                        ...
                        <div class="flex flex-col gap-3 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="radio" name="brand" value="" class="w-5 h-5 text-indigo-600 border-slate-300 focus:ring-indigo-600 rounded-full cursor-pointer" <?php echo e(!request('brand') ? 'checked' : ''); ?>>
                            ...
                            <label class="flex items-center gap-3 cursor-pointer group">
                                <input type="radio" name="brand" value="<?php echo e($brand->id); ?>" class="w-5 h-5 text-indigo-600 border-slate-300 focus:ring-indigo-600 rounded-full cursor-pointer" <?php echo e(request('brand') == $brand->id ? 'checked' : ''); ?>>
                                <span class="text-sm font-medium text-slate-600 group-hover:text-indigo-600 transition-colors"><?php echo e($brand->name); ?></span>
                            </label>
                        ...
                        <div class="flex flex-col gap-4">
                            <div class="grid grid-cols-2 gap-3">
                                <div class="relative">
                                    ...
                                    <input type="number" name="min_price" class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-2.5 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm font-bold text-slate-700" placeholder="0" value="<?php echo e(request('min_price')); ?>">
                                </div>
                                <div class="relative">
                                    <span class="absolute top-1/2 left-3 -translate-y-1/2 text-xs font-bold text-slate-400">Đến</span>
                                    <input type="number" name="max_price" class="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-3 py-2.5 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm font-bold text-slate-700" placeholder="Max" value="<?php echo e(request('max_price')); ?>">
                                </div>
                            </div>
                    ...
                    <div class="flex flex-col gap-3">
                        ...
                        <a href="<?php echo e(route('products.category', $categoryInfo['slug'])); ?>" class="w-full flex items-center justify-center gap-2 bg-slate-50 text-slate-600 font-bold px-6 py-3.5 rounded-xl hover:bg-slate-100 hover:text-rose-500 transition-colors">
                            Xóa bộ lọc
                        </a>
                    </div>
                </form>
            ...
            <main class="lg:col-span-3 flex flex-col gap-6">
                ...
                <div class="bg-white rounded-[2rem] shadow-sm shadow-slate-200/50 border border-slate-100 p-4 sm:p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-sm font-medium text-slate-500">
                        Hiển thị <strong class="text-slate-800"><?php echo e($products->firstItem() ?? 0); ?>-<?php echo e($products->lastItem() ?? 0); ?></strong>
                        trong <strong class="text-slate-800"><?php echo e($products->total()); ?></strong> sản phẩm
                    </div>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2 relative">
                            ...
                            <select name="sort" id="sortSelect" class="appearance-none bg-slate-50 border border-slate-200 rounded-xl pl-4 pr-10 py-2.5 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-sm font-bold text-slate-700 cursor-pointer" onchange="updateSort(this.value)">
                                ...
                                <option value="popular" <?php echo e(request('sort') == 'popular' ? 'selected' : ''); ?>>Phổ biến nhất</option>
                ...
                <div class="mt-8 flex justify-center">
                    <?php echo e($products->links('Customers.Components.pagination')); ?>
                ...
                <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-12 text-center flex flex-col items-center justify-center mt-4">
                    ...
                    <a href="<?php echo e(route('products.category', $categoryInfo['slug'])); ?>" class="bg-slate-900 text-white font-bold px-8 py-3.5 rounded-xl hover:bg-indigo-600 transition-colors shadow-lg shadow-slate-900/20">
                        Xóa tất cả bộ lọc
                    </a>
                </div>
...
<?php echo $__env->make('Customers.Layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/flashtm/Documents/FlashTechMongo/resources/views/Customers/Products/category.blade.php ENDPATH**/ ?>