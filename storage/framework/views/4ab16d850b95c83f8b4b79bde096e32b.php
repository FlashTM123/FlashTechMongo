<?php $__env->startSection('title', 'Sản phẩm yêu thích - FlashTech Premium'); ?>
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($wishlistProducts->count() > 0): ?>
                <div class="bg-white px-5 py-2.5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="font-black text-lg text-slate-800 leading-none"><?php echo e($wishlistProducts->count()); ?> <span class="text-sm font-medium text-slate-500">sản phẩm</span></div>
                    </div>
                </div>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($wishlistProducts->count() > 0): ?>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 sm:gap-6">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $wishlistProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                    <div class="group bg-white rounded-3xl p-4 sm:p-5 shadow-sm hover:shadow-xl hover:shadow-indigo-500/10 border border-slate-100 transition-all duration-300 relative flex flex-col h-full" id="wishlist-item-<?php echo e($product->_id); ?>">
                        
                        <!-- Remove Btn -->
                        <button class="absolute top-3 right-3 w-8 h-8 bg-white/80 backdrop-blur-sm border border-slate-100 rounded-full flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-colors shadow-sm z-10" onclick="removeWishlist('<?php echo e($product->_id); ?>')" title="Xóa khỏi yêu thích">
                        ...
                        <?php
                            $price = current_price($product); // using helper if exists, or manual calculation
                        ...
                        <!-- Image -->
                        <a href="<?php echo e(route('product.detail', $product->slug)); ?>" class="block relative aspect-square bg-slate-50 rounded-2xl mb-4 overflow-hidden">
                            <img src="<?php echo e($product->image ?? 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?w=200&h=200&fit=crop'); ?>" alt="<?php echo e($product->name); ?>" class="w-full h-full object-contain p-4 group-hover:scale-110 transition-transform duration-500">
                        </a>
                        ...
                        <div class="flex-1 flex flex-col">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($product->brand): ?>
                                <div class="text-[10px] sm:text-xs font-bold text-indigo-600 uppercase tracking-widest mb-1"><?php echo e($product->brand->name); ?></div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <a href="<?php echo e(route('product.detail', $product->slug)); ?>" class="text-sm sm:text-base font-bold text-slate-800 line-clamp-2 leading-tight group-hover:text-indigo-600 transition-colors mb-2"><?php echo e($product->name); ?></a>
                            
                            <div class="flex flex-col mt-auto pb-4 border-b border-slate-100 mb-4">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($discount > 0): ?>
                                    <span class="text-xs sm:text-sm text-slate-400 line-through font-medium"><?php echo e(number_format($basePrice, 0, ',', '.')); ?>₫</span>
                                    <span class="text-base sm:text-lg font-black text-rose-600 leading-none"><?php echo e(number_format($salePrice, 0, ',', '.')); ?>₫</span>
                                <?php else: ?>
                                    <span class="text-base sm:text-lg font-black text-slate-900 leading-none"><?php echo e(number_format($basePrice, 0, ',', '.')); ?>₫</span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>

                            <div class="flex items-center justify-between gap-2">
                                <span class="text-[10px] sm:text-xs font-bold <?php echo e($product->stock_quantity > 0 ? 'text-emerald-500 bg-emerald-50' : 'text-rose-500 bg-rose-50'); ?> px-2 py-1 rounded-md">
                                    <?php echo e($product->stock_quantity > 0 ? 'Còn hàng' : 'Hết hàng'); ?>

                                </span>
                                <button class="w-10 h-10 shrink-0 bg-slate-900 text-white rounded-xl flex items-center justify-center hover:bg-indigo-600 transition-colors shadow-md shadow-slate-900/10 disabled:opacity-50 disabled:hover:bg-slate-900" <?php echo e($product->stock_quantity <= 0 ? 'disabled' : ''); ?> onclick="addToCartFromWishlist('<?php echo e($product->_id); ?>')" title="Thêm vào giỏ">
...
<?php echo $__env->make('Customers.Layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/flashtm/Documents/FlashTechMongo/resources/views/Customers/Wishlist/index.blade.php ENDPATH**/ ?>