<?php $__env->startSection('title', 'Chỉnh sửa hồ sơ - FlashTech Premium'); ?>
...
<div class="bg-slate-50 min-h-screen pb-20 relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 pt-4 sm:pt-8 md:pt-12 relative z-10 max-w-4xl">
        ...
        <div class="flex items-center justify-between mb-8">
            ...
            <a href="<?php echo e(route('customers.profile.detail')); ?>" class="flex items-center gap-2 px-5 py-2.5 bg-white text-slate-600 font-bold rounded-xl shadow-sm border border-slate-200 hover:text-indigo-600 hover:border-indigo-600 transition-colors">
        ...
        <form action="<?php echo e(route('customers.profile.update')); ?>" method="POST" enctype="multipart/form-data" class="flex flex-col gap-8">
            <?php echo csrf_field(); ?>
                ...
                <div class="flex flex-col sm:flex-row items-center gap-8">
                    <div class="w-28 h-28 sm:w-32 sm:h-32 rounded-full border-4 border-slate-100 bg-gradient-to-tr from-indigo-500 to-purple-500 flex items-center justify-center overflow-hidden shrink-0 shadow-inner relative group" id="avatar-preview">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($customer->profile_picture_url): ?>
                            <img src="<?php echo e($customer->profile_picture_url); ?>" alt="<?php echo e($customer->full_name); ?>" class="w-full h-full object-cover" id="preview-img">
                        <?php else: ?>
                            <div class="text-4xl font-black text-white" id="preview-placeholder"><?php echo e(substr($customer->full_name, 0, 1)); ?></div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <div class="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" value="<?php echo e($customer->customer_code); ?>" class="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 outline-none font-bold text-slate-500 cursor-not-allowed" disabled>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Email</label>
                        <input type="email" value="<?php echo e($customer->email); ?>" class="w-full bg-slate-100 border border-slate-200 rounded-xl px-4 py-3 outline-none font-bold text-slate-500 cursor-not-allowed" disabled>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Họ và tên <span class="text-rose-500">*</span></label>
                        <input type="text" name="full_name" value="<?php echo e(old('full_name', $customer->full_name)); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-medium" required>
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Số điện thoại</label>
                        <input type="tel" name="phone_number" value="<?php echo e(old('phone_number', $customer->phone_number)); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-medium">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Ngày sinh</label>
                        <input type="date" name="date_of_birth" value="<?php echo e(old('date_of_birth', $customer->date_of_birth)); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-800 text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="gender" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-800 text-sm cursor-pointer appearance-none">
                            ...
                            <option value="male" class="font-bold text-slate-800 py-2" <?php echo e(old('gender', $customer->gender) == 'male' ? 'selected' : ''); ?>>Nam</option>
                            <option value="female" class="font-bold text-slate-800 py-2" <?php echo e(old('gender', $customer->gender) == 'female' ? 'selected' : ''); ?>>Nữ</option>
                            <option value="other" class="font-bold text-slate-800 py-2" <?php echo e(old('gender', $customer->gender) == 'other' ? 'selected' : ''); ?>>Khác</option>
                        </select>
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-sm font-bold text-slate-700 mb-2">Địa chỉ</label>
                        <input type="text" name="address" value="<?php echo e(old('address', $customer->address)); ?>" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all font-bold text-slate-800 placeholder:text-slate-400 placeholder:font-medium" placeholder="Số nhà, đường, phường/xã, quận/huyện...">
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end gap-4 mt-2">
                <a href="<?php echo e(route('customers.profile.detail')); ?>" class="px-8 py-4 bg-white text-slate-600 font-bold rounded-xl border border-slate-200 shadow-sm hover:border-slate-300 hover:text-slate-900 transition-colors">
                    Hủy bỏ
                </a>
...
<?php echo $__env->make('Customers.Layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/flashtm/Documents/FlashTechMongo/resources/views/Customers/Account/edit_profile.blade.php ENDPATH**/ ?>