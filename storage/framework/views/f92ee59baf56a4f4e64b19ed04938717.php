<header class="main-header sticky top-0 z-[1000] w-full transition-all duration-300 glass-effect border-b border-white/20">
    ...
    <div class="bg-slate-900 border-b border-white/10 text-white text-xs py-1.5 hidden md:block">
        <div class="container mx-auto px-4 flex justify-between items-center">
            ...
            <div class="flex items-center gap-4 relative z-[1001]">
                <?php echo $__env->make('customers.components.language-switcher', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            </div>
        </div>
    </div>
    ...
    <div class="container mx-auto px-4 py-3 md:py-4 relative z-[1000]">
        <div class="flex items-center justify-between gap-4 md:gap-8">
            <!-- Logo -->
            <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-2 group shrink-0">
            ...
            <div class="flex-1 max-w-2xl relative hidden md:block search-container group z-[1002]">
                <form action="<?php echo e(route('home')); ?>" method="GET" class="relative flex items-center bg-slate-100/80 hover:bg-white border border-slate-200/60 focus-within:border-indigo-400 focus-within:bg-white focus-within:ring-4 focus-within:ring-indigo-100 rounded-2xl transition-all duration-300 focus-within:shadow-md" id="mainSearchForm">
                    ...
                    <input type="text" name="search" placeholder="Tìm kiếm iPhone 15, Laptop Gaming..." class="w-full bg-transparent border-none focus:ring-0 text-sm py-3 text-slate-800 placeholder-slate-400 search-input font-medium" autocomplete="off" value="<?php echo e(request('search')); ?>">
                    <div class="pr-2 pl-2 border-l border-slate-200">
                        <select name="category" class="bg-transparent border-none text-sm text-slate-600 font-semibold py-2 focus:ring-0 cursor-pointer outline-none hover:text-indigo-600 transition-colors">
                            ...
                            <option value="Laptop" <?php echo e(request('category') === 'Laptop' ? 'selected' : ''); ?>>Laptop</option>
                            <option value="Tablet" <?php echo e(request('category') === 'Tablet' ? 'selected' : ''); ?>>Tablet</option>
                            <option value="Accessory" <?php echo e(request('category') === 'Accessory' ? 'selected' : ''); ?>>Phụ kiện</option>
                        </select>
                    </div>
                </form>
            ...
            <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
                ...
                <?php $comparisonCount = count(session()->get('comparison', [])); ?>
                <a href="<?php echo e(route('comparison.index')); ?>" class="relative p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50/50 rounded-xl transition-all hover-float group hidden lg:flex flex-col items-center justify-center h-12 w-12 comparison-action">
                    ...
                    <span class="absolute top-1 right-1 bg-rose-500 text-white text-[0.65rem] font-bold px-1.5 min-w-[1.25rem] rounded-full shadow-sm border border-white comparison-count text-center leading-tight" style="<?php echo e($comparisonCount > 0 ? '' : 'display:none'); ?>"><?php echo e($comparisonCount); ?></span>
                </a>

                <?php $wishlistCount = auth('customer')->check() ? count(auth('customer')->user()->wishlist ?? []) : 0; ?>
                <a href="<?php echo e(route('wishlist.index')); ?>" class="relative p-2 text-slate-500 hover:text-pink-600 hover:bg-pink-50/50 rounded-xl transition-all hover-float group hidden sm:flex flex-col items-center justify-center h-12 w-12">
                    ...
                    <span class="absolute top-1 right-1 bg-pink-500 text-white text-[0.65rem] font-bold px-1.5 min-w-[1.25rem] rounded-full shadow-sm border border-white wishlist-count text-center leading-tight" style="<?php echo e($wishlistCount > 0 ? '' : 'display:none'); ?>"><?php echo e($wishlistCount); ?></span>
                </a>

                <?php $cartCount = array_sum(array_column(session()->get('cart', []), 'quantity')); ?>
                <a href="<?php echo e(route('cart.index')); ?>" class="relative p-2 text-slate-500 hover:text-amber-600 hover:bg-amber-50/50 rounded-xl transition-all hover-float group flex flex-col items-center justify-center h-12 w-12 cart-action">
                    ...
                    <span class="absolute top-1 right-0 sm:right-1 bg-amber-500 text-white text-[0.65rem] font-bold px-1.5 min-w-[1.25rem] rounded-full shadow-sm border border-white cart-count text-center leading-tight transition-transform" style="<?php echo e($cartCount > 0 ? '' : 'display:none'); ?>"><?php echo e($cartCount); ?></span>
                </a>

                <div class="w-px h-6 bg-slate-200 mx-1 hidden sm:block"></div>

                <!-- Notifications Widget -->
                <?php echo $__env->make('customers.components.notifications-widget', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth('customer')->check()): ?>
                    <div class="dropdown dropdown-end z-[1005]">
                        <button type="button" tabindex="0" class="flex items-center gap-2 p-1 bg-white hover:bg-slate-50 border border-slate-200 rounded-full transition-all shadow-sm user-link cursor-pointer hover:shadow-md outline-none" id="userDropdownBtn">
                            <?php $avatar = auth('customer')->user()->profile_picture_url; ?>
                            <img src="<?php echo e($avatar ?: 'https://ui-avatars.com/api/?name='.urlencode(auth('customer')->user()->full_name).'&background=6366f1&color=fff'); ?>" alt="User" class="w-9 h-9 rounded-full border-2 border-white shadow-sm object-cover">
                            <span class="text-sm font-bold text-slate-700 hidden lg:block max-w-[100px] truncate pr-1"><?php echo e(auth('customer')->user()->full_name); ?></span>
                        ...
                        <div tabindex="0" class="dropdown-content mt-2 w-72 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl shadow-slate-200/50 border border-white overflow-hidden" id="userDropdownMenu">
                            <div class="p-5 bg-gradient-to-br from-indigo-500 via-purple-500 to-fuchsia-500 text-white relative overflow-hidden">
                                ...
                                <div class="flex items-center gap-3 relative z-10">
                                    <img src="<?php echo e($avatar ?: 'https://ui-avatars.com/api/?name='.urlencode(auth('customer')->user()->full_name).'&background=ffffff&color=4f46e5'); ?>" alt="User" class="w-14 h-14 rounded-full border-2 border-white/30 shadow-lg">
                                    <div>
                                        <div class="font-bold text-lg leading-tight tracking-tight"><?php echo e(auth('customer')->user()->full_name); ?></div>
                                        <div class="text-xs text-indigo-50 opacity-90 truncate max-w-[160px] font-medium"><?php echo e(auth('customer')->user()->email); ?></div>
                                        <div class="mt-2 inline-flex items-center gap-1 bg-black/20 backdrop-blur-md px-2.5 py-1 rounded-lg text-xs font-bold text-yellow-300 shadow-inner">
                                            ...
                                            <?php echo e(number_format(auth('customer')->user()->loyalty_points ?? 0)); ?> Điểm
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="py-2 px-2 z-[1006]">
                                <a href="<?php echo e(route('customers.profile.detail')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-slate-700 rounded-xl hover:bg-slate-50 hover:text-indigo-600 transition-colors dropdown-item">
                                ...
                                <a href="<?php echo e(route('customers.orders')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-slate-700 rounded-xl hover:bg-slate-50 hover:text-indigo-600 transition-colors dropdown-item">
                                ...
                                <a href="<?php echo e(route('wishlist.index')); ?>" class="sm:hidden flex items-center gap-3 px-3 py-2.5 text-sm font-semibold text-slate-700 rounded-xl hover:bg-slate-50 hover:text-pink-600 transition-colors dropdown-item">
                                ...
                                <form action="<?php echo e(route('customers.logout')); ?>" method="POST">
                                    <?php echo csrf_field(); ?>
                ...
                <?php else: ?>
                    <div class="hidden sm:flex items-center gap-3 ml-2">
                        <a href="<?php echo e(route('customers.login')); ?>" class="text-sm font-bold text-slate-600 hover:text-indigo-600 transition-colors px-2">Đăng nhập</a>
                        <a href="<?php echo e(route('customers.register')); ?>" class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-bold px-5 py-2.5 rounded-full shadow-lg shadow-indigo-500/30 hover:shadow-indigo-500/50 hover:-translate-y-0.5 transition-all">Đăng ký</a>
                    </div>
    ...
    <div class="hidden md:block relative z-[999] bg-white/70 backdrop-blur-md border-t border-slate-200/50">
        <div class="container mx-auto px-4">
            <ul class="flex items-center gap-8 text-[0.9rem] font-bold text-slate-600">
                <li>
                    <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-2 py-3.5 text-indigo-600 border-b-2 border-indigo-600">
                ...
                <li class="relative group">
                    ...
                    <div class="absolute top-full left-0 pt-2 w-64 hidden group-hover:block z-[1000] opacity-0 group-hover:opacity-100 transition-opacity">
                        <div class="bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-white p-3 overflow-hidden">
                            <a href="<?php echo e(route('products.category', 'smartphone')); ?>" class="group/item flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors mb-1">
                            ...
                            <a href="<?php echo e(route('products.category', 'laptop')); ?>" class="group/item flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors mb-1">
                            ...
                            <a href="<?php echo e(route('products.category', 'tablet')); ?>" class="group/item flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors mb-1">
                            ...
                            <a href="<?php echo e(route('products.category', 'accessory')); ?>" class="group/item flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-slate-50 transition-colors">
    ...
    if (searchInput && searchSuggestions && suggestionsList) {
        searchInput.addEventListener('input', (e) => {
            ...
            searchTimeout = setTimeout(async () => {
                try {
                    const response = await fetch(`<?php echo e(route('products.search')); ?>?search=${encodeURIComponent(query)}`);
...
<div class="fixed inset-0 z-[2000] lg:hidden bg-slate-900/50 backdrop-blur-sm opacity-0 invisible transition-all duration-300" id="mobileMenuOverlay">
    <div class="absolute inset-y-0 left-0 w-[280px] bg-white shadow-2xl -translate-x-full transition-transform duration-300 flex flex-col h-full" id="mobileMenuPanel">
        ...
        <div class="flex-1 overflow-y-auto py-4">
            <!-- Search Mobile -->
            <div class="px-4 mb-6">
                <form action="<?php echo e(route('home')); ?>" method="GET" class="relative flex items-center bg-slate-100 border border-slate-200 rounded-xl focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 transition-all">
                    <button type="submit" class="pl-3 pr-2 text-slate-400">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </button>
                    <input type="text" name="search" placeholder="Tìm kiếm..." class="w-full bg-transparent border-none focus:ring-0 text-sm py-2.5 text-slate-800 placeholder-slate-400" value="<?php echo e(request('search')); ?>">
                </form>
            </div>
            ...
            <div class="px-4 space-y-1">
                ...
                <a href="<?php echo e(route('home')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-slate-700 font-bold hover:bg-indigo-50 hover:text-indigo-600 rounded-xl transition-colors">
                ...
                <a href="<?php echo e(route('products.category', 'smartphone')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-slate-700 font-bold hover:bg-slate-50 transition-colors rounded-xl">
                    <div class="w-8 h-8 rounded-lg bg-slate-50 text-slate-500 flex items-center justify-center">📱</div> Điện thoại
                </a>
                <a href="<?php echo e(route('products.category', 'laptop')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-slate-700 font-bold hover:bg-slate-50 transition-colors rounded-xl">
                    <div class="w-8 h-8 rounded-lg bg-slate-50 text-slate-500 flex items-center justify-center">💻</div> Laptop
                </a>
                <a href="<?php echo e(route('products.category', 'tablet')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-slate-700 font-bold hover:bg-slate-50 transition-colors rounded-xl">
                    <div class="w-8 h-8 rounded-lg bg-slate-50 text-slate-500 flex items-center justify-center">📲</div> Máy tính bảng
                </a>
                <a href="<?php echo e(route('products.category', 'accessory')); ?>" class="flex items-center gap-3 px-3 py-2.5 text-slate-700 font-bold hover:bg-slate-50 transition-colors rounded-xl">
            ...
            <div class="px-4 mt-8">
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <button type="button" onclick="switchLanguage(event, '<?php echo e($code); ?>')" class="flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-slate-200 text-sm font-bold transition-all w-full <?php echo e(is_locale($code) ? 'bg-indigo-50 border-indigo-200 text-indigo-600 shadow-sm' : 'hover:bg-slate-50 text-slate-600'); ?>">
                        <span><?php echo e($locale['flag']); ?></span> <?php echo e(strtoupper($code)); ?>
        ...
        <div class="p-4 border-t border-slate-100 bg-slate-50">
            <div class="grid grid-cols-2 gap-3">
                <a href="<?php echo e(route('customers.login')); ?>" class="px-4 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold text-sm text-center rounded-xl shadow-sm hover:text-indigo-600 hover:border-indigo-200 transition-colors">Đăng nhập</a>
                <a href="<?php echo e(route('customers.register')); ?>" class="px-4 py-2.5 bg-indigo-600 text-white font-bold text-sm text-center rounded-xl shadow-md shadow-indigo-500/30 hover:bg-indigo-700 transition-colors">Đăng ký</a>
            </div>
        </div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>
...
<?php /**PATH /home/flashtm/Documents/FlashTechMongo/resources/views/Customers/Layouts/navbar.blade.php ENDPATH**/ ?>